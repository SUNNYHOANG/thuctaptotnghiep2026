...
const CTSVNhacNho = () => {
  ...
  const loadLookupData = async () => {
    try {
      ...
      setKhoaList(khoaData.map((k) => ({ makhoa: k.name })));

      setLopList(lopRes.data?.data || []);
    } catch (err) {
  ...
  // Realtime: khi có nhắc nhở mới → reload history
  useSocketEvent('thongbao:new', loadHistory);

  const handlePreview = async () => {
    if (!filterHocky) { setPreviewError('Vui lòng chọn học kỳ trước khi xem trước.'); return; }
    if (!filterTrangthai) { setPreviewError('Vui lòng chọn trạng thái DRL.'); return; }
    setPreviewing(true);
    setPreviewError('');
    setPreviewList(null);
    try {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.tieude?.trim()) { setMessage('Vui lòng nhập tiêu đề.'); setMessageType('error'); return; }
        ...
        <div className="filter-row">
          <div className="filter-field">
            ...
            <select value={filterHocky} onChange={(e) => setFilterHocky(e.target.value)}>
              ...
              {hockyList.map((hk) => (
                <option key={hk.mahocky} value={String(hk.mahocky)}>{hk.tenhocky || hk.mahocky}</option>
              ))}
            </select>
          </div>
          <div className="filter-field">
          ...
          <div className="filter-field">
            ...
            <select value={filterKhoa} onChange={(e) => setFilterKhoa(e.target.value)}>
              ...
              {khoaList.map((k) => (
                <option key={k.makhoa} value={k.makhoa}>{k.makhoa}</option>
              ))}
            </select>
          </div>
          <div className="filter-field">
            ...
            <select value={filterLop} onChange={(e) => setFilterLop(e.target.value)}>
              ...
              {lopList.map((l) => (
                <option key={l.malop} value={l.malop}>{l.tenlop ? `${l.malop} – ${l.tenlop}` : l.malop}</option>
              ))}
            </select>
          </div>
        </div>
      ...
      {previewList !== null && (
        <section className="preview-section">
          ...
          {previewList.length === 0 ? (
            <div className="message warning">⚠️ Không có sinh viên nào phù hợp với bộ lọc đã chọn.</div>
          ) : (
            <div className="table-wrapper">
              <table className="preview-table">
                <thead>
                  <tr><th>#</th><th>MSSV</th><th>Họ tên</th><th>Lớp</th><th>Khoa</th><th>Trạng thái</th></tr>
                </thead>
                <tbody>
                  {previewList.map((sv, idx) => (
                    <tr key={sv.mssv}>
                      <td>{idx + 1}</td><td>{sv.mssv}</td><td>{sv.hoten}</td>
                      <td>{sv.malop}</td><td>{sv.makhoa}</td><td>{sv.trangthai || filterTrangthai}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section className="reminder-form-section">
        ...
        <form onSubmit={handleSubmit} className="nhac-nho-form">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (val === '__custom__') {
                ...
                if (tpl) { setForm({ tieude: tpl.tieude, noidung: tpl.noidung, loai: tpl.loai }); setSelectedTemplate(tpl.id); }
              }
            }}
          >
            <option value="">-- Chọn tiêu đề --</option>
            {REMINDER_TEMPLATES.map((tpl) => (
              <option key={tpl.id} value={tpl.tieude}>{tpl.tieude}</option>
            ))}
            <option value="__custom__">✏️ Tự nhập tiêu đề...</option>
          </select>
          {!REMINDER_TEMPLATES.some((t) => t.tieude === form.tieude) && (
            <input value={form.tieude} onChange={(e) => setForm((f) => ({ ...f, tieude: e.target.value }))}
              placeholder="Nhập tiêu đề thông báo" maxLength={255} style={{ marginTop: '6px' }} />
          )}
          <label>Nội dung</label>
          <textarea value={form.noidung} onChange={(e) => setForm((f) => ({ ...f, noidung: e.target.value }))}
            rows={5} placeholder="Nội dung nhắc nhở" />
          ...
          <div className="form-actions">
            <button type="submit" className="btn primary" disabled={sending || !canSend}
        ...
        {historyLoading ? (
          ...
          <div className="table-wrapper">
            <table className="history-table">
              <thead>
                <tr>
                  <th>Thời gian gửi</th><th>Tiêu đề</th><th>Loại</th><th>Số người nhận</th><th>Người gửi</th>
                </tr>
              </thead>
              <tbody>
                {historyRows.map((row) => (
