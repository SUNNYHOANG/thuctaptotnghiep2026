      ...
      {loading ? (
        ...
        <div className="scores-table-wrapper">
          <table className="scores-table">
            ...
            <tbody>
              {scores.map(score => {
                const total = score.diemtong ?? 0;
                const level = getScoreLevel(total || 0);
                return (
                  <tr key={score.madiemrenluyen || `${score.mssv}-${score.mahocky}`}>
                    <td>{score.tenhocky}</td>
                    <td>{score.namhoc}</td>
                    <td className="text-center">{Number(score.diemhoatdong || 0).toFixed(1)}</td>
                    <td className="text-center">{Number(score.diemhoctap || 0).toFixed(1)}</td>
                    <td className="text-center">{Number(score.diemkyluat || 0).toFixed(1)}</td>
                    <td className={`text-center font-bold level-${level.color}`}>
                      {Number(total || 0).toFixed(1)}
                    </td>
                    <td className={`text-center font-bold level-${level.color}`}>
                      {level.label}
                    </td>
